          ...
          <div class="content lg:pl-6 mb-6">
            <div class="tabcontent">
              <div class="h-[340px] md:h-[600px] overflow-hidden rounded-20">
                <img class="rounded-20" :src="activeItem" :alt="data.title" />
              </div>
            </div>
          </div>
        </div>
        <div class="col-span-4">
          <div class="tabs">
            <div v-for="(item, index) in data.features" :key="index"
              ...
              :class="activeItem == item.image.url ? 'bg-blue-500' : '' " @click="activeItem = item.image.url">
              <h3 class="font-medium text-2xl mb-4 duration-300"
                :class="activeItem == item.image.url ? 'text-white' : 'text-dark'">
                {{ item.title }}
              </h3>
              <p class="test-base font-light duration-300"
                :class="activeItem == item.image.url ? 'text-white opacity-80' : 'text-dark-gray'">
                {{ item.description }}
              </p>
            </div>
          </div>
