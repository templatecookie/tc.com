        ...
        <div
          ...
          <div class="flex justify-start lg:w-0 lg:flex-1">
            <nuxt-link to="/">
              ...
              <img class="h-8 w-auto sm:h-10" src="/logo-white.svg"
      ...
      <div class="mx-auto max-w-7xl px-4 sm:px-6" v-if="product.menuItems && product.menuItems.length">
        <nav class="hidden md:flex md:justify-between py-1 pb-4">
          <div class="flex flex-wrap">
            <a :href="item.href" v-for="(item, index) in product.menuItems" :key="index"
              class="text-sm text-gray-400 hover:text-gray-900 hover:bg-gray-200 rounded-xl py-2 px-4">
              {{ item.label }}
            </a>
          </div>
          <div class="flex">
            <nuxt-link v-if="product.docsUrl" :to="product.docsUrl"
              class="text-sm text-gray-900 bg-gray-100 rounded-xl py-2 px-4">Documentation</nuxt-link>
          </div>
        </nav>
      </div>
      ...
      <div class="absolute inset-x-0 top-0 origin-top-right transform p-2 transition md:hidden z-40" v-if="mobileMenu">
        <div class="divide-y-2 divide-gray-50 rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5">
          <div class="px-5 pt-5 pb-6">
            ...
            <div class="mt-4 border-t-2 border-gray-100 pt-4">
              <nav class="grid gap-y-4">
                <a :href="item.href" class="flex items-center rounded-md hover:bg-gray-50"
                  v-for="(item, index) in product.menuItems" :key="index">
          ...
          <div class="space-y-6 py-6 px-5">
            <div>
              <nuxt-link v-for="(item, index) in mainMenuItems" :key="index" :to="item.href"
